...
function useWatch(props) {
	...
	const defaultValueMemo = import_react.useMemo(() => control._getWatch(name, _defaultValue.current), [control, name]);
	const [value, updateValue] = import_react.useState(_compute.current ? _compute.current(defaultValueMemo) : defaultValueMemo);
	useIsomorphicLayoutEffect(() => control._subscribe({
		name,
...
function useController(props) {
	...
	const defaultValueMemo = import_react.useMemo(() => get(control._formValues, name, get(control._defaultValues, name, defaultValue)), [
		control,
		name,
		defaultValue
	]);
...
* ```
*/
var Watch = ({ control, names, render }) => render(useWatch({
	control,
	name: names
}));
